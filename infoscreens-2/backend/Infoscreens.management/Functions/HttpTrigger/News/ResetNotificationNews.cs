...
{
    public class ResetNotificationNews : BaseApiClass
    {
        ...
        const string FUNCTION_NAME = "ResetNotificationNews";
        [Function(FUNCTION_NAME)]
        public async Task<HttpResponseData> RunAsync([HttpTrigger(AuthorizationLevel.Anonymous, "post", "put", Route = "v1/news/{tenantCode}/{newsId}/resetNotification")] HttpRequestData req, string tenantCode, int newsId)
        {
            try
            {
                ...
                // Reset notification flag and persist news
                var news = await _databaseRepository.GetNewsFromTenantAsync(tenant, newsId);
                news = await _newsRepository.ResetNewsNotificationAsync(news, user);
