...
export const _getIconName: (openWeatherIconName: string) => {
    class: string;
    color: string;
} = (openWeatherIconName: string) => {
    switch (openWeatherIconName) {
        case '01d':
            return { class: 'wi-day-sunny', color: '#fac029' };
        case '01n':
            return { class: 'wi-night-clear', color: '#fac029' };
        case '02d':
            return { class: 'wi-day-cloudy', color: '#ec974a' };
        case '02n':
            return { class: 'wi-night-alt-cloudy', color: '#ec974a' };
        case '03d':
            return { class: 'wi-cloud', color: '#b4b4b4' };
        case '03n':
            return { class: 'wi-cloud', color: '#b4b4b4' };
        case '04d':
            return { class: 'wi-cloudy', color: '#7e7e7e' };
        case '04n':
            return { class: 'wi-cloudy', color: '#7e7e7e' };
        case '09d':
            return { class: 'wi-hail', color: '#008bc8' };
        case '09n':
            return { class: 'wi-hail', color: '#008bc8' };
        case '10d':
            return { class: 'wi-day-hail', color: '#4cb4f0' };
        case '10n':
            return { class: 'wi-night-alt-hail', color: '#4cb4f0' };
        case '11d':
            return { class: 'wi-thunderstorm', color: '#000000' };
        case '11n':
            return { class: 'wi-thunderstorm', color: '#000000' };
        case '13d':
            return { class: 'wi-snow-wind', color: '#69d5df' };
        case '13n':
            return { class: 'wi-snow-wind', color: '#69d5df' };
        case '50d':
            return { class: 'wi-fog', color: '#b4b4b4' };
        case '50n':
            return { class: 'wi-fog', color: '#b4b4b4' };
    }
    return { class: '', color: '' };
...
export class DailyWeatherComponent implements AfterViewInit {
    slide: Slide = Slide.WeatherDaily;
    weather: any;
    hourlyTemps: any;
    hourlyDates: any;
    ...
    @ViewChild('DailyWeatherChart')
    DailyWeatherChart: ElementRef;
    ngAfterViewInit() {
        try {
            ...
            this.http.get('cache/' + LocalCacheFileName.OpenWeather).subscribe(data => {
                ...
                const ctx = this.DailyWeatherChart.nativeElement;
                ...
                LoggingService.debug(DailyWeatherComponent.name, 'Preparing slide', 'Done');
            });
        } catch (error) {
            LoggingService.error(DailyWeatherComponent.name, 'Preparing slide', 'An error occured while getting the slide data', error);
        }
    }
