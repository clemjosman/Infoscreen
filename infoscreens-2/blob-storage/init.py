...
if __name__ == "__main__":
    ...
    # Only initialize if not already initialized (Azure function will create a container "azure-webjobs-hosts" automatically before this check)
    container_list = blob_service_client.list_containers()
    container_1 = next(container_list, None)
    container_2 = next(container_list, None)
    if (container_2 is not None) and (container_1.name != container_2.name):
        print(f"Emulator already has containers named {container_2.name}, will skip initialization.", flush=True)
    else:
